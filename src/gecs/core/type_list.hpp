...
template <typename List, template <typename> typename F>
struct list_filter_not;

template <template <typename...> typename ListType, typename T, typename... Ts,
          template <typename> typename F>
struct list_filter_not<ListType<T, Ts...>, F> {
    using type = std::conditional_t<
        !F<T>::value,
        list_add_to_first_t<typename list_filter_not<ListType<Ts...>, F>::type, T>,
        typename list_filter_not<ListType<Ts...>, F>::type>;
};

template <template <typename...> typename ListType,
          template <typename> typename F>
struct list_filter_not<ListType<>, F> {
    using type = ListType<>;
};
...
template <typename List, template <typename> typename F>
using list_filter_not_t = typename detail::list_filter_not<List, F>::type;
...
struct find_first<Idx, Max, T, type_list<Ts...>> {
    static constexpr size_t value =
        std::is_same_v<T, list_element_t<type_list<Ts...>, Idx>>
            ? Idx
            : find_first<Idx + 1, Max, T, type_list<Ts...>>::value;
};
