...
Texture& TextureManager::Load(const std::string& name,
    ...
    return *textures_
                .emplace(name, std::make_unique<Texture>(
                                   renderer_, filename, keycolor, std::nullopt))
                .first->second;
}
...
const Tilesheet* TextureManager::FindTilesheet(const std::string& name) const {
...
Image::Image(const Texture& texture, Rect rect) : texture_(&texture), rect_(rect) {}
