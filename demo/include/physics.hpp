...
inline float RectCollidTime(const Rect& src, const Rect& dst,
                            const Vector2& src_vel) {
    ...
    float x_percent =
        src_vel.x == 0
            ? 0
            : std::min(
                  (dst.position.x + dst.size.w + half_src_size.w) / src_vel.x,
                  (dst.position.x - half_src_size.w) / src_vel.x);
    float y_percent =
        src_vel.y == 0
            ? 0
            : std::min(
                  (dst.position.y + dst.size.h + half_src_size.h) / src_vel.y,
